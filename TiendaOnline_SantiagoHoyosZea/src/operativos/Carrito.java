	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getSession().getAttribute("usuario")!=null) {
			...
			response.getWriter().println(
					...
					"							<br>\n" +
					"							<br>\n" +
					"							<a href=\"/TiendaOnline_SantiagoHoyosZea/index.jsp?accion=login\"\"><button>INICIAR SESI&Oacute;N</button></a>\n" +
					"						</div>\n" +
					"					</center>\n" +
					"				</div>\n" +
					"			</div>");
	...
	private void pintarCarrrito(HttpServletRequest req, PrintWriter pintor,  HashMap<Integer, Integer> carrito) {
		...
		if (carrito.size()>0) {
			...
			for (int id_producto: carrito.keySet()) {
				...
				pintor.println(
						"	<tr>\n" +
						"    <td class=\"tg-k9ij\"><a href=\"/TiendaOnline_SantiagoHoyosZea/index.jsp?accion=carrito&p="+p.getId()+"\" />"
			...
			pintor.println(
					...
					"							<br>\n" +
					"							<a href=\"/TiendaOnline_SantiagoHoyosZea/index.jsp?accion=index\"\"><button>IR A COMPRAR ALGO</button></a>\n" +
					"						</div>\n" +
					"					</center>\n" +
					"				</div>\n" +
					"			</div>");
