...
public class GestorProductos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "mod":
			...
			printFormularioProductos(response.getWriter(),p, "MODIFICACI&Oacute;N DE PRODUCTO",hacer);
			break;
		...
		c.close();
			...
			response.getWriter().println(
					...
					"							<br>\n" +
					"							<br>\n" +
					"							<a href=\"/TiendaOnline_SantiagoHoyosZea/index.jsp?accion=admin\"\"><button>VOLVER</button></a>\n" +
					"						</div>\n" +
					"					</center>\n" +
					"				</div>\n" +
					"			</div>");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			switch (confirma) {
			case "add":
				GestorBD.getInstance().insertarProdcuto(c, p);
...
"							<form action=\"/TiendaOnline_SantiagoHoyosZea/index.jsp\" method=\"post\" >\n" +
...
"									  <option value=\"vestible\" "+((p!=null && p.getCategoria().equals("vestible")?"selected=\"selected\"":""))+">Vestible</option>\n" +
"									  <option value=\"accesorio\" "+((p!=null && p.getCategoria().equals("accesorio")?"selected=\"selected\"":""))+">Accesorio</option>\n" +
"									  <option value=\"ultimo\" "+((p!=null && p.getCategoria().equals("ultimo")?"selected=\"selected\"":""))+">Ultimas existencias</option>\n" +
"									  <option value=\"software\" "+((p!=null && p.getCategoria().equals("software")?"selected=\"selected\"":""))+">Software</option>\n" +
"									  <option value=\"soporte\" "+((p!=null && p.getCategoria().equals("soporte")?"selected=\"selected\"":""))+">Soporte</option>\n" +
"									  </select></td></tr>\n" +
"							<tr><td colspan=\"2\">&nbsp;</td></tr>\n" +
